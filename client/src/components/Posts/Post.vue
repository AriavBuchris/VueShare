            ...
            <v-flex xs12>
                <v-card hover>
                    <v-card-title>
                        ...
                        <v-spacer></v-spacer>
                        <v-icon @click="goToPreviousPage" large color="info">arrow_back</v-icon>
                    </v-card-title>
                    <v-tooltip right>
            ...
            <v-layout class="mb-3" v-if="user">
                <v-flex xs12>
                    <v-form v-model="isFormValid" lazy-validation ref="form" @submit.prevent="handleAddPostMessage">
                        <v-layout row>
                            <v-flex xs12>
                                <v-text-field :rules="messageRules" v-model="messageBody" clearable prepend-icon="email" :append-outer-icon="messageBody && 'send'" 
                                label="Add Message" type="text" @click:append-outer="handleAddPostMessage" required></v-text-field>
                            </v-flex>
                        </v-layout>
                    </v-form>
                </v-flex>
